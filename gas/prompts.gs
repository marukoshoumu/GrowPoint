...
function buildStage2PromptHardcoded_(userMaster, glossaryEntries, previousIssues) {
  ...
  return 'あなたは就労継続支援B型事業所の支援記録を構造化するAIアシスタントです。\n\n'
    ...
    + '8. monitoring_sheet_evidence の各項目キーに、そのキーの意味に合ったevidenceのみが入っているか？（下記ルール参照）\n\n'
    + '【monitoring_sheet_evidence の配分ルール】\n'
    + '各キーには、そのキーが表す項目に直接関係するevidenceのみを記載してください。\n'
    + '1つの項目に複数の情報を詰め込まないでください。\n\n'
    + '【monitoring_sheet_evidence の書式ルール — 重要】\n'
    + 'evidence フィールドには、発言原文（「」付き）をそのまま入れないでください。\n'
    + '「である」調の要約文で記載してください。1項目あたり80字以内を目安とします。\n'
    + '例: ✕「今のペースを崩さずにできればいいかな。」→ ○ 現在の作業ペースに疲労感はなく、維持したいとの意向あり。\n'
    + '例: ✕「偏頭痛が夜になるとひどくなるし。薬飲まないと寝れない。」→ ○ 夜間の偏頭痛がひどく、服薬しないと入眠できない。\n\n'
    + 'work_life:\n'
    + '  attendance → 出勤・欠勤・遅刻・早退に関する事実のみ\n'
    + '  punctuality → 時間厳守（開始・終了時刻）に関する事実のみ\n'
    + '  health_management → 健康管理・服薬・通院・睡眠・体調に関する事実のみ\n'
    + '  appearance → 身だしなみに関する事実のみ\n'
    + '  rule_compliance → 規則遵守に関する事実のみ\n'
    + '  reporting → 報告・連絡・相談・SOSの発信に関する事実のみ\n'
    + '  workspace_tidiness → 整理整頓に関する事実のみ\n'
    + '  work_attitude → 作業への意欲・積極性に関する事実のみ\n'
    + '  concentration → 集中力に関する事実のみ\n'
    + '  persistence → 持続力・最後までやり遂げることに関する事実のみ\n\n'
    + 'relationships:\n'
    + '  greeting → 挨拶に関する事実のみ\n'
    + '  conversation → 同僚との会話・コミュニケーションに関する事実のみ\n'
    + '  understanding_hierarchy → 上司・指揮命令系統の理解に関する事実のみ\n'
    + '  emotional_control → 感情コントロール・怒り・泣きに関する事実のみ\n'
    + '  stress_management → ストレス対処・蓄積に関する事実のみ\n\n'
    + 'tasks:\n'
    + '  physical_stamina → 体力・疲労・持久力に関する事実のみ\n'
    + '  instruction_compliance → 指示理解・遵守に関する事実のみ\n'
    + '  quality → 作業の完成度・正確さに関する事実のみ\n'
    + '  speed → 作業速度に関する事実のみ\n'
    + '  safety_awareness → 安全意識・道具の扱いに関する事実のみ\n\n'
    + '該当するキーがない情報は monitoring_sheet_evidence に無理に入れず、categories のみに記載してください。\n\n'
    + '【JSON出力の厳密なルール】\n'
...
function buildStage3APromptHardcoded_(userMaster, extractionJson) {
  ...
  return 'あなたは就労継続支援B型事業所の支援記録を作成するAIアシスタントです。\n\n'
    ...
    + '【簡潔さのルール — 最重要】\n'
    + '- 記録票は担当者が読んで加筆修正する「下書き」です。面談内容を網羅的に書き起こすのではなく、支援に必要な要点のみを簡潔に記載してください\n'
    + '- 同じ事実を複数セクションで繰り返さないでください。最も適切なセクション1箇所にのみ記載してください\n'
    + '- 各セクションの目安文字数（厳密な上限ではなく目安）:\n'
    + '  - セクション1（本人・家族の意向）: 本人150字以内、家族50字以内\n'
    + '  - セクション4（支援の実施状況）: 短期目標ごとに200字以内、その他の状況200字以内\n'
    + '  - セクション5（感想）: 150字以内\n'
    + '  - セクション6（支援方針）: 合意事項150字以内、所見200字以内\n'
    + '  - セクション7（特記事項）: 150字以内。他セクションで既に記載した内容は繰り返さない\n'
    + '- 面談中の全発言を盛り込む必要はありません。支援上の判断に必要な核心的事実を選んでください\n\n'
    + '【過去の事業所の情報の扱い — 重要】\n'
    + '抽出JSONの time_context フィールドを確認し、以下のルールで記載してください：\n'
    + '- time_context="current" → そのまま記載（グローポイントでの出来事）\n'
    + '- time_context="past_facility" → 必要な場合のみ簡潔に言及し「（過去：○○事業所）」と明記。facility_name が "不明" の場合は「（過去：以前の事業所）」\n'
    + '- time_context="general" → そのまま記載（継続的な事実）\n'
    + '- 過去の事業所での作業内容の詳細（具体的な作業名の羅列、商品化の詳細等）は省略し、支援に直接関係する要点のみ記載してください\n'
    + '- グローポイントでの現在の状況を中心に記載し、過去の事業所の情報はあくまで補足として扱ってください\n\n'
    + '【利用者マスター情報】\n'
    + `- 利用者名: ${userMaster.name}\n`
    + `- 担当者名: ${userMaster.staff}\n`
    ...
    + '（設定がある場合は、①と同様に記載）\n\n'
    + '【その他の状況（身体面・生活面）】\n'
    + '（いずれの短期目標にも直接紐づかないが、支援上重要な身体面・生活面の情報を簡潔に記載）\n'
    + '（200字以内を厳守。支援判断に直接影響する事実のみ。以下は記載不要：手帳・年金の種類一覧、ヘルパー支援の分単位の内訳、過去の事業所での具体的作業名リスト、料理の内容）\n'
    + '（手帳・年金・通院先等の事務的情報はセクション7に簡潔に記載）\n\n'
    + '---\n\n'
    ...
    + '（セクション4で既に記載した支援内容・身体面の詳細はここでは繰り返さない。本人の「感想・気持ち」に絞る）\n\n'
    + '---\n\n'
    + '## 6. 今後の支援方針（短期目標の「終了」「継続」「内容変更」等）\n\n'
    + 'データソース: cat7_agreements + cat6_staff (sub_type: "advice" | "concern")\n'
    + 'applicable_sections: "monitoring_6_future"\n\n'
    + '【合意事項・次回までの取り決め】\n'
    + '（cat7_agreements の内容を箇条書きで簡潔に記載。セクション4の内容を繰り返さない）\n\n'
    + '【支援者の所見】\n'
    + '（cat6_staff の観察・懸念を記載。セクション4で既出の事実は省略し、支援者固有の見解・懸念のみ）\n\n'
    ...
    + '（注意: 他のセクション（1〜6）で既に記載した内容はここに繰り返さない。セクション1〜6に収まらなかった残りの重要情報のみ）\n'
    + '（過去の事業所での作業詳細・経歴の羅列は不要。支援上の注意点に絞る）\n\n'
    + '---\n\n'
    + '文書末尾に以下の注記を必ず付与：\n'
