...
#### 長時間音声を分割するとき（目安：約20分ごと）
Google Apps Script の実行時間上限のため、**長尺は複数ファイルに分ける**運用を推奨します。ファイル名の末尾に **`_NN-MM`** を付けます（**2桁の数字**、NN=何番目、MM=分割数）。
...
※ `AUDIO_SPLIT_ENABLED=true` のとき、ワーカー URL とシークレットは必須。ワーカーのビルド・デプロイは `workers/audio-split/README.md` を参照。
