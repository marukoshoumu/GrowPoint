...
Gemini API を利用しています。**デフォルトのモデル**（`gas/config.gs` の `CONFIG.GEMINI_MODEL`）は **`gemini-2.5-flash-lite`** で、軽量・高速な呼び出し向けです。**Stage 2（構造化抽出）** と **Stage 3** の一部は **`gemini-2.5-flash`**（`STAGE2_MODEL` 等）を明示的に指定しています。

**文字起こし（旧 Stage 1 の実処理）** は GAS の実行時間制限を避けるため、**Cloud Run 上の `workers/transcribe` ワーカー**（Gemini File API + Cloud Tasks）で実行します。GAS は **`/enqueue` でジョブ投入**し、Drive 上の **`…_文字起こし.txt` / チャンク別 `…_文字起こし_NN.txt` をポーリング**して `STAGE1_PENDING` → `STAGE1_DONE`（またはチャンク待ち・マージ）へ進めます。ワーカー側の既定モデルは `gemini-2.5-flash`（`workers/transcribe` の環境変数 `GEMINI_MODEL`）です。

**リポジトリ構成**

| パス | 内容 |
|------|------|
| `gas/` | Apps Script 本体（`clasp` でプッシュ） |
| `workers/transcribe/` | 文字起こし Cloud Run（Flask、Docker） |
| `workers/audio-split/` | 長尺 **単一ファイル**の時間分割（Cloud Run + ffmpeg、任意） |
...
graph TD
    ...
    subgraph "システム内部（自動処理パイプライン）"
        C --> S1["Stage 1: 文字起こし<br/>Cloud Run transcribe"]
        S1 --> S2["Stage 2: 構造化抽出<br/>GAS + Gemini"]
        S2 --> S3["Stage 3: ドキュメント生成<br/>GAS + Gemini"]
        S3 --> D
    end
...
#### 長時間音声を扱うとき（2通り）
**A. 手動チャンク（複数ファイル）**  
長尺は **複数ファイルに分ける**運用が基本です。拡張子を除いたファイル名の **末尾** に **`_NN-MM`** を付けます（NN=何番目、MM=分割総数。数字は **可変桁**。例として 2 桁で書くことも多いです）。
...
GAS は同一グループの **`STAGE1_PENDING` をチャンク番号順（01→02）に処理**し、先にマージが走って `_01.txt` だけ消える不整合を防いでいます（`gas/dashboard.gs` のソート）。
...
| カテゴリ | プロパティ名 | 必須 | 説明 |
|:---|:---|:---:|:---|
| **基本設定** | `GEMINI_API_KEY` | はい | Gemini API キー（GAS から Stage2/3 等で使用） |
| | `SPREADSHEET_ID` | はい | スプレッドシート ID |
| | `TEMPLATE_ID_MONITORING_DOCUMENT` | はい | モニタリング記録票のドキュメント ID |
| **文字起こし（Cloud Run）** | `TRANSCRIBE_WORKER_URL` | はい* | 例: `https://transcribe-xxxxx.run.app`（末尾 `/` 不要） |
| | `TRANSCRIBE_AUTH_SECRET` | はい* | ワーカーと共有する Bearer トークン（十分に長いランダム文字列） |
...
\* 本番で文字起こしを Cloud Run に任せる場合は **`TRANSCRIBE_WORKER_URL`** と **`TRANSCRIBE_AUTH_SECRET`** が必須です。ビルド・デプロイ・IAM・Cloud Tasks は **`workers/transcribe/DEPLOY.md`** と **`workers/transcribe/README.md`** を参照してください。GAS は Bearer のみで `/enqueue` する想定のため、Cloud Run は **未認証 Invoker（公開）** にし、アプリ側でシークレット検証する構成が `audio-split` と整合します。
...
### 停滞時の手動リカバリ（メニュー）
スプレッドシートを開いた状態で、メニュー **「停滞ジョブを手動リカバリ」** を実行すると、時間閾値を待たずに次のような復旧を試みます（`gas/main.gs` の `manualRecoverStuckJobs`）。

- `STAGE1_PENDING` → `QUEUED`（リトライカウンタ維持、上限超過はスキップ）
- `STAGE2_RUNNING` / `STAGE3_RUNNING` → 前ステージへ戻す
- 再試行可能な `ERROR`（Stage2 一時失敗など）→ `STAGE1_DONE` に戻して Stage2 をやり直し
...
| プロパティ名 | 対象ステージ | 説明 |
|:---|:---|:---|
| `PROMPT_FILE_ID_STAGE1` | 文字起こし | 話者の特定や、誤字脱字の修正ルールを指示できます（ワーカーへ渡る本文） |
...
- **文字起こしが進まない（`STAGE1_PENDING` のまま）**: `TRANSCRIBE_WORKER_URL` / `TRANSCRIBE_AUTH_SECRET`、Cloud Run・Cloud Tasks のログ、Drive の `03` 配下に期待ファイル名の TXT ができるかを確認してください。長時間待てる場合は自動のタイムアウト回復もあります。**急ぎの場合はメニュー「停滞ジョブを手動リカバリ」**を利用できます。
- **Stage2 で MAX_TOKENS / JSON 切れ**: 長大トランスクリプトでは `maxOutputTokens` と **thinking 消費**の兼ね合いで出力が足りないことがあります。`CONFIG.STAGE2_THINKING_BUDGET`（`gas/config.gs`）と `extract.gs` の thinking 設定で緩和しています。根本対策はプロンプト短縮・分割抽出など（継続検討）。
- **Stage2 が実行時間切れ**: GAS の 6 分上限に達すると `handleError_` が走らず `STAGE2_RUNNING` のまま残ることがあります。一定時間後の自動回復や、手動リカバリで前段階に戻して再実行してください。
